...
exports.getRate = async (source, target) => {
  try {
    ...
    if (
      currencyRate &&
      moment().add(-1, "days").isBefore(currencyRate.updatedAt)
    ) {
      return currencyRate.rate;
    } else if (!currencyRate) {
      currencyRate = new DB.CurrencyRate({ source, target });
    }
    const uri = `https://free.currconv.com/api/v7/convert?q=${q}&compact=y&apiKey=${process.env.CURRENCY_CONVERT_API_KEY}`;
    return new Promise((resolve, reject) =>
      request(
        ...
        async (err, response, body) => {
          ...
          if(body.search("503 Service Unavailable")){
